<?php  

	if (strlen($_POST['c_key']) != 16 ) {
		header('location: index.php?msg=notCorrect');
	}
	$pText = $_POST['plain_text'];
	$cipherKey = $_POST['c_key'];

	$stringToAscii = unpack("C*", $cipherKey);
	$stringToAsciiPlain = unpack("C*", $pText);

	echo "<pre>";
	print_r($stringToAscii);
	echo "</pre>";
	$eightBitConv = array();

	$SBOX = array(
	    
	);

	//// index124
	$INV_SBOX = array(
	    
	);
    // echo "<ul>";
    // foreach ($SBOX as $val) {
    // 	echo "<li>".(int)$val."</li>";
    // }
    // echo "</ul>";
    $arrBin = array();

    $allKeys = array();

    $firstFour = array();
    $mid1Four = array();
    $mid2Four = array();
	$lastFour = array();

	foreach ($stringToAscii as $value) {
		$bin = decbin($value);	
		$len = strlen($bin);				
		$lackOfZeros = 8 - $len;
		
		for ($i=0; $i < $lackOfZeros; $i++) { 
			$bin = '0'.$bin;
		}

		array_push($arrBin, $bin);	
	}

		$initialKey = '';
		foreach ($arrBin as $value) {
			$initialKey .= $value;
		}

		// echo "<p>Initial Key: $initialKey</p>";
		array_push($allKeys, $initialKey);

		$i = 1;
	foreach ($arrBin as $val) {						
		if ($i <= 4) {
			array_push($firstFour, $val);	
		}		
		if ($i > 4 and $i <= 8) {
			array_push($mid1Four, $val);	
		}		
		if ($i > 8 and $i <= 12) {
			array_push($mid2Four, $val);	
		}		
		if ($i > 12) {
			array_push($lastFour, $val);	
		}		
		echo "<b>".$i++."</b>.$val<br>";	
	}


	echo "<br>W0 :";

	foreach ($firstFour as $val) {
		echo "$val&nbsp;&nbsp;";
	}

	echo "<br>W1 :";
	$W1 = '';
	foreach ($mid1Four as $val) {		
		$W1 .= $val;
	}
	echo $W1;

	echo "<br>W2 :";

	$W2 = '';
	foreach ($mid2Four as $val) {
		$W2 .= $val;
	}

	echo $W2;

	echo "<br>W3 :";

	$W3new = '';
	foreach ($lastFour as $val) {
		$W3new .= $val;
	}

	echo $W3new;

	echo "<br>";

	$W3 = array_shift($lastFour);
	array_push($lastFour, $W3);

	echo "<h1 style='color: red;'>After shifting the W3 first 8 bits: <br>";
	foreach ($lastFour as $var) {
		echo $var;
	}
	echo "</h1>";
	// echo "<br>";

	// Pushing the first 8 bit of W3 at the end and shifting 24 bits to the most right of the word
	// foreach ($lastFour as $val) {
	// 	echo "<u>$val</u>&nbsp;&nbsp;&nbsp;";
	// }

	// Substitution of the aquired values with the values aquired from the SBOX
	foreach($lastFour as $value) {
		$current = bindec($value);
		$curVal =  (int)$SBOX[$current];
		$curBin = decbin($curVal);

		array_shift($lastFour);
		array_push($lastFour, $curBin);
	}

	echo "<br>";

	foreach ($lastFour as $val) {		
		$len = strlen($val);				
		$lackOfZeros = 8 - $len;
		
		for ($i=0; $i < $lackOfZeros; $i++) { 
			$val = '0'.$val;
		}

		array_shift($lastFour);
		array_push($lastFour, $val);
			
	}

	echo "<h1>After substituting the values from sbox:<br>";
	foreach ($lastFour as $value) {
		echo "<u>$value </u>";
	}
	echo "</h1>";
	//$RC = array(0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36);

	$RC = array('00000001', '00000010', '00000100', '00001000','00001010', '00010100', '01000000', '10000000', '00011011', '00110110');

	echo "<h1>RC[0]: $RC[0]<br>lastFour[0]: $lastFour[0]";
	$G1 = (int)$RC[0] ^ (int)$lastFour[0];


	// echo "<br>";
	// echo "$RC[0]";
	// echo "<br>$lastFour[0]<br>"; 
	
	$lackOfZeros = 8 - strlen($G1);

	for ($i=0; $i < $lackOfZeros; $i++) { 
		$G1 = '0'.$G1;
	}

	echo "<br>First eight bits of G1: $G1<br>";

	array_shift($lastFour);
	array_unshift($lastFour, $G1);
	
	$G1 = '';
	foreach ($lastFour as $val) {
		$G1 .= $val;
	}

	echo "<h1>G 0: $G1</h1>";

	$W0 = '';
	foreach ($firstFour as $val) {
		$W0 .= $val;		
	}

	echo "<h1>W0: $W0</h1>";

	$W4 = '';
	echo "<hr>";
	echo "<h1>W4: ";
	for ($i=0; $i < 32; $i++) { 
		$W4 .= (int)$G1[$i] ^ (int)$W0[$i];		
	}
	echo "$W4</h1>";
	echo "<h1>W1: $W1</h1>";
	$W5 = '';
	echo "<hr>";
	echo "<h1>W5: ";
	for ($i=0; $i < 32; $i++) { 
		$W5 .= (int)$W1[$i] ^ (int)$W4[$i];		
	}
	echo "$W5</h1>";
	echo "<h1>W2: $W2</h1>";
	echo "<hr>";
	$W6 = '';
	echo "<h1>W6: ";
	for ($i=0; $i < 32; $i++) { 
		$W6 .= (int)$W2[$i] ^ (int)$W5[$i];		
	}
	echo "$W6</h1>";
	echo "<h1>W3: $W3new</h1>";

	echo "<hr>";
	$W7 = '';
	echo "<h1>W7: ";
	for ($i=0; $i < 32; $i++) { 
		$W7 .= (int)$W3new[$i] ^ (int)$W6[$i];		
	}
	echo "$W7</h1>";

	$key1 = '';
	$key1 .= $W4.$W5.$W6.$W7;
		
	array_push($allKeys, $key1);

	$arr2 = str_split($key1, 8);

	echo "<pre>";
	print_r($allKeys);
	echo "</pre>";

	echo "<pre>";
	print_r($arr2);
	echo "</pre>";

	//////////////////////////////////////////89gfhfdkgjkfbcvnbmnjgadfkg//////////////////////



	for ($j=0; $j < 9; $j++) { 

		$firstFour = array(); 
		$mid1Four = array(); 
		$mid2Four = array(); 
		$lastFour = array(); 

		echo "<h1 style='text-align: center;'>Epoch :".($j+1)."</h1>";
		$i = 1;
		foreach ($arr2 as $val) {						
			if ($i <= 4) {
				array_push($firstFour, $val);	
			}		
			if ($i > 4 and $i <= 8) {
				array_push($mid1Four, $val);	
			}		
			if ($i > 8 and $i <= 12) {
				array_push($mid2Four, $val);	
			}		
			if ($i > 12) {
				array_push($lastFour, $val);	
			}		
			echo "<b>".$i++."</b>.$val<br>";	
		}	

	echo "<pre>";
	print_r($lastFour);
	echo "</pre>";

	echo "<br>W0 :";

	foreach ($firstFour as $val) {
		echo "$val&nbsp;&nbsp;";
	}

	echo "<br>W1 :";
	$W1 = '';
	foreach ($mid1Four as $val) {		
		$W1 .= $val;
	}
	echo $W1;

	echo "<br>W2 :";

	$W2 = '';
	foreach ($mid2Four as $val) {
		$W2 .= $val;
	}

	echo $W2;

	echo "<br>W3 :";

	$W3new = '';
	foreach ($lastFour as $val) {
		$W3new .= $val;
	}

	echo $W3new;

	echo "<br>";

	$W3 = array_shift($lastFour);
	array_push($lastFour, $W3);

	echo "<h1 style='color: red;'>After shifting the W3 first 8 bits: <br>";
	foreach ($lastFour as $var) {
		echo $var;
	}
	echo "</h1>";
	// echo "<br>";

	// Pushing the first 8 bit of W3 at the end and shifting 24 bits to the most right of the word
	// foreach ($lastFour as $val) {
	// 	echo "<u>$val</u>&nbsp;&nbsp;&nbsp;";
	// }

	// Substitution of the aquired values with the values aquired from the SBOX
	foreach($lastFour as $value) {
		$current = bindec($value);
		$curVal =  (int)$SBOX[$current];
		$curBin = decbin($curVal);

		array_shift($lastFour);
		array_push($lastFour, $curBin);
	}

	echo "<br>";

	foreach ($lastFour as $val) {		
		$len = strlen($val);				
		$lackOfZeros = 8 - $len;
		
		for ($i=0; $i < $lackOfZeros; $i++) { 
			$val = '0'.$val;
		}

		array_shift($lastFour);
		array_push($lastFour, $val);
			
	}

	echo "<h1>After substituting the values from sbox:<br>";
	foreach ($lastFour as $value) {
		echo "<u>$value </u>";
	}
	echo "</h1>";
	//$RC = array(0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36);

	$RC = array('00000001', '00000010', '00000100', '00001000','00001010', '00010100', '01000000', '10000000', '00011011', '00110110');

	echo "<h1>RC[".($j+1)."]: ".$RC[$j+1]."<br>lastFour[0]: $lastFour[0]";
	// $G1 = (int)$RC[$j+1] ^ (int)$lastFour[0];        ///////////////////////ghadfgyryhiuwehfkhjhhyasiuewfdkjg
	$G1 = '';
	// echo "<u>".$lastFour[0]."</u>";
	for ($i=0; $i < 8; $i++) { 
		$G1 .= (int)$RC[$j+1][$i] ^ (int)$lastFour[0][$i];		
	}


	// echo "<br>";
	// echo "$RC[0]";
	// echo "<br>$lastFour[0]<br>"; 
	
	$lackOfZeros = 8 - strlen($G1);

	for ($i=0; $i < $lackOfZeros; $i++) { 
		$G1 = '0'.$G1;
	}

	echo "<br>First eight bits of G1: $G1<br>";

	array_shift($lastFour);
	array_unshift($lastFour, $G1);
	
	$G1 = '';
	foreach ($lastFour as $val) {
		$G1 .= $val;
	}

	echo "<h1>G 0: $G1</h1>";

	$W0 = '';
	foreach ($firstFour as $val) {
		$W0 .= $val;		
	}

	echo "<h1>W0: $W0</h1>";

	$W4 = '';
	echo "<hr>";
	echo "<h1>W4: ";
	for ($i=0; $i < 32; $i++) { 
		$W4 .= (int)$G1[$i] ^ (int)$W0[$i];		
	}
	echo "$W4</h1>";
	echo "<h1>W1: $W1</h1>";
	$W5 = '';
	echo "<hr>";
	echo "<h1>W5: ";
	for ($i=0; $i < 32; $i++) { 
		$W5 .= (int)$W1[$i] ^ (int)$W4[$i];		
	}
	echo "$W5</h1>";
	echo "<h1>W2: $W2</h1>";
	echo "<hr>";
	$W6 = '';
	echo "<h1>W6: ";
	for ($i=0; $i < 32; $i++) { 
		$W6 .= (int)$W2[$i] ^ (int)$W5[$i];		
	}
	echo "$W6</h1>";
	echo "<h1>W3: $W3new</h1>";

	echo "<hr>";
	$W7 = '';
	echo "<h1>W7: ";
	for ($i=0; $i < 32; $i++) { 
		$W7 .= (int)$W3new[$i] ^ (int)$W6[$i];		
	}
	echo "$W7</h1>";

	$key1 = '';
	$key1 .= $W4.$W5.$W6.$W7;
			
	array_push($allKeys, $key1);

	$arr2 = str_split($key1, 8);

	echo "<pre>";
	print_r($allKeys);
	echo "</pre>";

	echo "<pre>";
	print_r($arr2);
	echo "</pre>";
	}
///////////////////////////////////////////////////// Encryption /////////////////////////////////////////////////
	
	echo "<h1 style='text-align:center; font-size: 35px;'>Encryption Stages</h1>";
	echo "<pre>";	
	print_r($stringToAsciiPlain);
	echo "</pre>";

	$arrBin = array();    

    $firstFour = array();
    $mid1Four = array();
    $mid2Four = array();
	$lastFour = array();
	foreach ($stringToAsciiPlain as $value) {
		$bin = decbin($value);	
		$len = strlen($bin);				
		$lackOfZeros = 8 - $len;
		
		for ($i=0; $i < $lackOfZeros; $i++) { 
			$bin = '0'.$bin;
		}

		array_push($arrBin, $bin);	
	}

	echo "<pre>";	
	print_r($arrBin);
	echo "</pre>";	

	$plain_txt = '';
	foreach ($arrBin as $val) {
		$plain_txt .= $val;
	}

	echo "Plain: ".$plain_txt;
	echo "<br>Key: $allKeys[0]";
	for ($j=0; $j < 9; $j++) { 			
		$vals = '';
		// echo "<br>";
		// echo $allKeys[0];
		// echo "<br>";
		// echo $plain_txt;
		for ($i=0; $i < 128; $i++) { 
			$vals .= (int)$allKeys[$j][$i] ^ (int)$plain_txt[$i];
		}

		echo "<p>Key".$j.": ".$allKeys[$j]."</p>";
		echo "<p>KEY XOR Plain: ".$vals."</p>";

		$arr3 = str_split($vals, 8);

		echo "<pre>";	
		print_r($arr3);
		echo "</pre>";

		foreach($arr3 as $value) {
			$current = bindec($value);
			$curVal =  (int)$SBOX[$current];
			$curBin = decbin($curVal);

			array_shift($arr3);
			array_push($arr3, $curBin);
		}

		foreach ($arr3 as $val) {		
			$len = strlen($val);				
			$lackOfZeros = 8 - $len;
			
			for ($i=0; $i < $lackOfZeros; $i++) { 
				$val = '0'.$val;
			}

			array_shift($arr3);
			array_push($arr3, $val);
				
		}
		
		echo "<p>Subbytes</p>";
		echo "<pre>";	
		print_r($arr3);
		echo "</pre>";

		///////////////// Shift rows
		$temp = $arr3[1];
		$arr3[1] = $arr3[5];
		$arr3[5] = $arr3[9];
		$arr3[9] = $arr3[13];
		$arr3[13] = $temp;

		$temp = $arr3[2];
		$arr3[2] = $arr3[10];
		$arr3[10] = $temp;


		$temp = $arr3[6];
		$arr3[6] = $arr3[14];
		$arr3[14] = $temp;

		$temp = $arr3[15];
		$arr3[15] = $arr3[11];
		$arr3[11] = $arr3[7];
		$arr3[7] = $arr3[3];
		$arr3[3] = $temp;

		echo "<p>Shift Rows:</p>";
		echo "<pre>";	
		print_r($arr3);
		echo "</pre>";	

		$temp = $arr3[1];
		$arr3[1] = $arr3[4];
		$arr3[4] = $temp;

		$temp = $arr3[2];
		$arr3[2] = $arr3[8];
		$arr3[8] = $temp;

		$temp = $arr3[3];
		$arr3[3] = $arr3[12];
		$arr3[12] = $temp;

		$temp = $arr3[6];
		$arr3[6] = $arr3[9];
		$arr3[9] = $temp;

		$temp = $arr3[7];
		$arr3[7] = $arr3[13];
		$arr3[13] = $temp;

		$temp = $arr3[11];
		$arr3[11] = $arr3[14];
		$arr3[14] = $temp;

		echo "<p>After Transpose:</p>";
		echo "<pre>";	
		print_r($arr3);
		echo "</pre>";		

		$plain_txt = '';

		foreach ($arr3 as $value) {
			$plain_txt .= $value;
		}


		echo "Intemediate: $plain_txt";
	}

	$vals = '';
		// echo "<br>";
		// echo $allKeys[0];
		// echo "<br>";
		// echo $plain_txt;
		for ($i=0; $i < 128; $i++) { 
			$vals .= (int)$allKeys[9][$i] ^ (int)$plain_txt[$i];
		}

		echo "<p>Key 9: ".$allKeys[9]."</p>";
		echo "<p>KEY XOR Plain: ".$vals."</p>";

		$arr3 = str_split($vals, 8);

		echo "<pre>";	
		print_r($arr3);
		echo "</pre>";

		echo "<h1 style='font-size: 50px;'>";
		echo $arr3[8];
		echo "</h1>";

		foreach($arr3 as $value) {
			$current = bindec($value);
			$curVal =  (int)$SBOX[$current];
			$curBin = decbin($curVal);

			array_shift($arr3);
			array_push($arr3, $curBin);
		}

		foreach ($arr3 as $val) {		
			$len = strlen($val);				
			$lackOfZeros = 8 - $len;
			
			for ($i=0; $i < $lackOfZeros; $i++) { 
				$val = '0'.$val;
			}

			array_shift($arr3);
			array_push($arr3, $val);
				
		}
		
		echo "<p>Subbytes</p>";
		echo "<pre>";	
		print_r($arr3);
		echo "</pre>";

		///////////////// Shift rows
		$temp = $arr3[1];
		$arr3[1] = $arr3[5];
		$arr3[5] = $arr3[9];
		$arr3[9] = $arr3[13];
		$arr3[13] = $temp;

		$temp = $arr3[2];
		$arr3[2] = $arr3[10];
		$arr3[10] = $temp;


		$temp = $arr3[6];
		$arr3[6] = $arr3[14];
		$arr3[14] = $temp;

		$temp = $arr3[15];
		$arr3[15] = $arr3[11];
		$arr3[11] = $arr3[7];
		$arr3[7] = $arr3[3];
		$arr3[3] = $temp;

		echo "<p>Shift Rows:</p>";
		echo "<pre>";	
		print_r($arr3);
		echo "</pre>";			

		$plain_txt = '';

		foreach ($arr3 as $value) {
			$plain_txt .= $value;
		}


		echo "Intemediate: $plain_txt";

		$vals = '';
		for ($i=0; $i < 128; $i++) { 
			$vals .= (int)$allKeys[10][$i] ^ (int)$plain_txt[$i];
		}

		echo "<p>Cypher Text: $vals</p>";	

		////////////////////////////////// Decryption //////////////////////////////////
		if (isset($_POST['decrypt'])) {
			echo "<h1 style='text-align:center; font-size: 35px;'>Decryption Stages</h1>";

			$inter = '';
			for ($i=0; $i < 128; $i++) { 
				$inter .= (int)$allKeys[10][$i] ^ (int)$vals[$i];
			}


			$arr3 = array();

			$arr3 = str_split($inter, 8);

			echo "XOR:";
			echo "<pre>";
		    print_r($arr3);
		    echo "</pre>";


			$temp = $arr3[13];
		    $arr3[13] = $arr3[9];
		    $arr3[9]  = $arr3[5];
		    $arr3[5]  = $arr3[1];
		    $arr3[1] = $temp;
		    // row2
		    $temp = $arr3[14];
		    $arr3[14]  = $arr3[6];
		    $arr3[6] = $temp;

		    $temp = $arr3[10];
		    $arr3[10]  = $arr3[2];
		    $arr3[2] = $temp;
		    // row3
		    $temp = $arr3[3];
		    $arr3[3] = $arr3[7];
		    $arr3[7] = $arr3[11];
		    $arr3[11]  = $arr3[15];
		    $arr3[15]  = $temp;

		    echo "Inverse Shift:";
		    echo "<pre>";
		    print_r($arr3);
		    echo "</pre>";

		    foreach($arr3 as $value) {
				$current = bindec($value);
				$curVal =  (int)$INV_SBOX[$current];
				$curBin = decbin($curVal);

				array_shift($arr3);
				array_push($arr3, $curBin);
			}

			foreach ($arr3 as $val) {		
				$len = strlen($val);				
				$lackOfZeros = 8 - $len;
				
				for ($i=0; $i < $lackOfZeros; $i++) { 
					$val = '0'.$val;
				}

				array_shift($arr3);
				array_push($arr3, $val);
			}


			echo "Inverse Sub:";
			echo "<pre>";
			print_r($arr3);
			echo "</pre>";

			$vals = '';		

			foreach ($arr3 as $value) {
				$vals .= $value;
			}


			echo "<p>Key 9: ";
			echo $allKeys[9];
			echo "</p>";

			echo "<p>Subbyte: $vals</p>";

			$values = '';
			for ($i=0; $i < 128; $i++) { 
				$values .= (int)$allKeys[9][$i] ^ (int)$vals[$i];
			}

			echo "<p>XOR: ";
			echo $values;		
			echo "</p>";

			echo "<pre>sdfjkd";
			print_r(str_split($values, 8));
			echo "</pre>";

			$arr3 = array();
			$arr3 = str_split($values, 8);

			for ($j=8; $j >= 0; $j--) { 

				//// Transpose
				$temp = $arr3[1];
				$arr3[1] = $arr3[4];
				$arr3[4] = $temp;

				$temp = $arr3[2];
				$arr3[2] = $arr3[8];
				$arr3[8] = $temp;

				$temp = $arr3[3];
				$arr3[3] = $arr3[12];
				$arr3[12] = $temp;

				$temp = $arr3[6];
				$arr3[6] = $arr3[9];
				$arr3[9] = $temp;

				$temp = $arr3[7];
				$arr3[7] = $arr3[13];
				$arr3[13] = $temp;

				$temp = $arr3[11];
				$arr3[11] = $arr3[14];
				$arr3[14] = $temp;						

				
				// //// inverse shift
				$temp = $arr3[13];
			    $arr3[13] = $arr3[9];
			    $arr3[9]  = $arr3[5];
			    $arr3[5]  = $arr3[1];
			    $arr3[1] = $temp;

			    
			    $temp = $arr3[14];
			    $arr3[14]  = $arr3[6];
			    $arr3[6] = $temp;


			    $temp = $arr3[10];
			    $arr3[10]  = $arr3[2];
			    $arr3[2] = $temp;

			    
			    $temp = $arr3[3];
			    $arr3[3] = $arr3[7];
			    $arr3[7] = $arr3[11];
			    $arr3[11]  = $arr3[15];
			    $arr3[15]  = $temp;


			 //    //// Inverse SubBytes

			    foreach($arr3 as $value) {
					$current = bindec($value);
					$curVal =  (int)$INV_SBOX[$current];
					$curBin = decbin($curVal);

					array_shift($arr3);
					array_push($arr3, $curBin);
				}

				foreach ($arr3 as $val) {		
					$len = strlen($val);				
					$lackOfZeros = 8 - $len;
					
					for ($i=0; $i < $lackOfZeros; $i++) { 
						$val = '0'.$val;
					}

					array_shift($arr3);
					array_push($arr3, $val);
				}

				$values = '';		

				foreach ($arr3 as $value) {
					$values .= $value;
				}					

				// echo "<pre>Inverse Sub";
				// print_r($arr3);						
				// /// Key XOR
				

				$vals = '';
				for ($i=0; $i < 128; $i++) { 
					$vals .= (int)$allKeys[$j][$i] ^ (int)$values[$i];
				}

				echo "<p>";
				echo $vals;
				echo "</p>";

				$arr3 = array();

				$arr3 = str_split($vals, 8);			
			}

			echo "<pre>";
			print_r($arr3);
			echo "</pre>";

			foreach ($arr3 as $value) {
				$binToDec = bindec($value);

				array_shift($arr3);
				array_push($arr3, $binToDec);
			}

			echo "<pre>";
			print_r($arr3);
			echo "</pre>";		

			$decrypt = '';

			foreach ($arr3 as $value) {
				$decrypt .= chr($value);
			}		

			echo "<h1>$decrypt</h1>";

			header('location: index.php');
				
		}		
?>